...
void fightWidget::oneFight(QWidget* parent)
{
    ...
    if(!diff)
    {//普通关卡
        right[0] = new juniorDemon(fight, ":/image/demon.png", QPoint(1000, 650), 100, num[0] % 5 + 1 + (100 - light_value) / 20, num[1] % 5 + 1, 10, num[2] % 5 + 1, num[3] % 5 + 1, 0, 1);
        right[1] = new juniorDemon(fight, ":/image/monster_1.png", QPoint(1000, 450), 100, num[4] % 5 + 1, num[5] % 5 + 1 + (100 - light_value) / 20, 10, num[6] % 5 + 1, num[7] % 5 + 1, 0, 1);
        right[2] = new juniorDemon(fight, ":/image/demon.png", QPoint(1000, 250), 100, num[0] % 5 + 1, num[1] % 5 + 1, 10, num[2] % 5 + 1 + (100 - light_value) / 20, num[3] % 5 + 1 + (100 - light_value) / 20, 0, 1);
    }
    else
    {//精英关卡
        right[0] = nullptr;
        right[2] = nullptr;
        right[1] = new eliteDemon(fight, ":/image/monster_0.png",QPoint(1000, 450), 300 - light_value, 10 + (100 - light_value) / 20, 10 + (100 - light_value) / 20, 15, 9 + (100 - light_value) / 20, 9 + (100 - light_value) / 20, 5, 3);
    }
    //道具效果
